...
// usage: node render.mjs [--only 0,72,180] [--grid 24] [--all]
...
const out = path.join(root, 'out'), frames = path.join(out, 'frames'); mkdirSync(frames, {recursive: true});
...
try {
  ...
  await page.goto(`http://127.0.0.1:${port}/rossen-live-intro.html?bare=1`);
...
if (argv.includes('--all')) {
  ...
  console.log('video: out/video.mp4');
}
