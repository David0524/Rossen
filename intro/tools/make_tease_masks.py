"""Masks for the Friday tease's last bar: the LIVE TODAY loop's pieces (logo, LIVE TODAY, the time card, FRIDAY, LIVE ON YOUTUBE, Jeff),
...
LOOP = 'out/rossen-loop-friday-youtube'   # the loop the tease hands off into (the Friday loop with its LIVE ON YOUTUBE chip)
...
for i in range(102, 120):
    f = np.array(Image.open(f'{LOOP}/frames/{i:04d}.png').convert('RGB')).astype(int); U |= np.abs(f - P).sum(2) > 30
