...
- **Amazon promo codes (bars 8–9):** WE FOUND HIDDEN / AMAZON PROMO CODES.
  ...
  - Three code tickets slide out from behind the logo card, then REVEALED LIVE stamps cover the codes. No real codes are shown.
- **Live (bar 10):** LIVE ON YOUTUBE / ROSSEN REPORTS. A plain, unbranded video player shows Jeff live, above a ROSSEN REPORTS CHANNEL row and a SEE YOU AT 10! chat bubble.
- **Handoff (bar 11):** the camera drops onto the loop's own page. Its pieces pop in on the beats, cut from the loop's own frames with `assets/tease/loop_mask_*.png` (made by `tools/make_tease_masks.py`). The last tease frame is loop frame 119, so the cut into the loop is the loop's own seamless wrap.

Audio: `score_tease.py` composes bars 0–9. Bars 10–11 play the loop's own cue, so the handoff is sample-exact. Build everything with `tools/build_tease.sh`.
